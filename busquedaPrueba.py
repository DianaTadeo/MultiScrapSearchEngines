...
def ip(ip,obj_search,browser):
	...
	query=''
	if search=='':
		if browser in ['Google', 'DuckDuckGo', 'Bing', 'Yahoo', 'Baidu']:   query+='ip%3A'+ip+'&oq=ip%'+ip
	else:
		if browser in ['Google', 'DuckDuckGo', 'Bing', 'Yahoo', 'Baidu']:   query+='ip%3A'+ip+'+'+obj_search
	return query

def filetype(tipo_archivo,obj_search,browser):
	#p=inurl%3A".pdf"+algo
	query=''
	if browser in ['Google', 'Bing', 'Baidu']:	query += 'filetype%3A'+tipo_archivo+'+'+obj_search
...
def site(site,obj_search,browser):
	...
	query=''
	if search=='':
		if browser in ['Google', 'DuckDuckGo', 'Bing', 'Yahoo', 'Baidu']:   query+='site%3A' + site+'&oq=site%3A'+site
	else:
		if browser in ['Google', 'DuckDuckGo', 'Bing', 'Yahoo', 'Baidu']:   query+='site%3A' + site+'+'+obj_search
	return query
